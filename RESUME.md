...
1. Review enabled services and decide Cirrus host identity: server with temporary GNOME, or workstation with services.
2. Resolve Phoenix as staging-only vs durable service data, then document the directory and ownership model.
3. Capture a fresh post-cleanup state snapshot after any hardening or service changes.
