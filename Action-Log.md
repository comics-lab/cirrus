...
- Imported the external `fearless-ssh/LIBRARY/FOUND_FILES` markdown handoff set back into the repo under `imports/fearless-library-handoff-2026-04-22/`, then reconciled it against live Cirrus/Mylar state. The staged `Archie (1960)` missing issues `151, 191, 237, 261, 266, 269, 489` and `Black Hood Comics (1943) #15` were copied into the live library, rescanned with Mylar, and verified in the DB as fully `Downloaded`; `MISSING_ISSUES.md` is now empty.
- Added `utilities/dc_finest_metadata.py` and used it to enrich three `DC Finest` books with rooted `ComicInfo.xml` plus `MetronInfo.xml` using ISBN/GCD-oriented metadata instead of ComicVine IDs, then moved them into `/mnt/phoenix/media/comics/DC Comics/DC Finest/` for Kavita/library use rather than Mylar import.
- Cleaned the live JDownloader intake again: removed empty directories, extracted and deleted the remaining `.zip` files, archived the last redundant raw `Unearth` `.cbr`, and re-established the active raw queue boundary at `98` `.cbz`, `0` `.cbr`, `0` `.zip`, and `4` `.pdf`.
- Added `utilities/seed_legacy_series_metadata.py` and patched `utilities/cv_issue_resolver.py` so it now trusts parent `series.json` ComicVine `comicid` values before fuzzy series guessing, and normalizes malformed year strings into real 4-digit years.
- Proved the legacy-library plus GCD seeding lane on JDownloader. These runs were seeded with `series.json`, `cvinfo`, and rooted `ComicInfo.xml`, then resolved to issue-level ComicVine IDs and re-audited as fully `mylar_valid`: `Ultimate Elektra` `5 / 5`, `Mantra v1` `24 / 24`, `Mantra v2` `7 / 7`, `Superboy & The Ravers` `19 / 19`, `Marvel Mutts Infinity Comic` `2 / 2`, and `Batgirl` `122 / 122`.
- Promoted those `179` newly `mylar_valid` files from JDownloader into `/mnt/phoenix/media/incoming/mylar-import`, raising the staged basket to `182` `.cbz` including three earlier TPB-style books already there.
- Reset and verified Mylar’s folder monitor against `/mylar-imports`: `config.ini` still points `check_folder`, `manual_pp_folder`, and `comic_dir` at `/mylar-imports`, and a container restart confirmed the 5-minute folder monitor is active.
- Observed that blind Mylar folder consumption does not drain that `182`-file basket. Repeated scans only reprocessed already-known series like `Superboy & The Ravers` and `Marvel Mutts Infinity Comic`, primarily as duplicate checks, without moving the basket forward.
- Reduced `/mnt/phoenix/media/incoming/mylar-import` back to three standalone books by moving `179` known-series files out to `/mnt/phoenix/media/incoming/archive/mylar-import-cleanup/2026-04-26-known-series-duplicates`. The remaining active basket is now:
  - `52 Aftermath - The Four Horsemen v01`
